    ...
    <fin-portlet>
      <fin-portlet-header>
        <fin-portlet-heading :loading="loading" backArrow>
          <span class="User_heading">Batch Channel</span>
        </fin-portlet-heading>
        <fin-portlet-item>
      ...
      <q-dialog v-model="dialogOpen" persistent>
        <q-card>
          <q-card-section>
            <div class="text-h6">Edit Batch Number</div>
          </q-card-section>
    ...
    <fin-portlet>
      <fin-portlet-header>
        <fin-portlet-heading :loading="loading">
          <span class="User_heading">Active Channels</span>
        </fin-portlet-heading>
        <fin-portlet-item>
          <div class="row q-col-gutter-md">
            <q-select
...
import '@quasar/extras/material-icons/material-icons.css';
  ...
  export default {
    ...
    data() {
      return {
        ...
        header: [
          ...
          { label: 'Email Address', key: 'email', align: 'start' },
          { label: 'Role', key: 'role', align: 'start' },
          { label: 'Batch', key: 'batch', align: 'start' },
        ],
        userData:'',
        batch:0,
        batchData:[],
        currentBatch:null,
        selectedBatchChat:null,
        searchBatch: "", 
        searchName: "",
        selectedBatch: null,
    ...
    computed: {
      batchOptions() {
        const uniqueBatches = [...new Set(this.batchData.map(item => item.batch))];
    ...
    filteredBatchData() {
      return this.batchData.filter(item => {
        const nameMatch = !this.searchName || item.name.toLowerCase().includes(this.searchName.toLowerCase());
        const batchMatch = !this.selectedBatch || item.batch === this.selectedBatch.batchNumber;
        return nameMatch && batchMatch;
      });
    },
  },
  watch: {
    selectedBatchChat(newVal) {
      // When the selected batch changes, update batchNumber
      this.batchNumber = newVal.batchNumber.replace(/\D/g, '');
    ...
    methods: {
      ...
      this.$api.get(url1).then(response => {
        ...
        if (response.data.success) {
          ...
          this.sendUserDataToBatch();
          if (this.userData==='') {
            this.showMsg("No user data found", 'info');
          }
        } else {
    ...
    async sendUserDataToBatch() {
    this.loading = true; // Start loading before the loop

    // Create a set of existing emails for quick lookup
    const existingEmails = new Set(this.batchData.map(item => item.email.trim().toLowerCase()));

    for (const user of this.userData) {
      if (!existingEmails.has(user.email.trim().toLowerCase())) { // Check if user is not already in batch
        const userData = {
          name: user.name || '',
        ...
        try {
          const response = await this.$api.post('/api/batchdata', userData);

          if (!response.data) {
            this.showMsg(response.data?.message || `Failed to send data for ${user.name}`, 'negative');
          } 
        } catch (error) {
          //this.showMsg(error.response?.data.message || error.message, 'negative');
    ...
    this.loading = false; // Stop loading after the loop
  },
async getBatchData() {
    this.loading = true; // Start loading indicator

    try {
        const response = await this.$api.get('/api/getBatchData'); // Adjust the URL if necessary
        
        //console.log(response.data)
        if (response.data) {
            this.batchData = response.data.map((item, index) => ({
            ...item,
            index: index + 1,
            name:item.name,
            role: item.role,
      ...
      editDataFun(val) {
        this.currentBatch = parseInt(val.batch.replace("Batch ", ""), 10);
        this.selectedBatchemail= val.email;
        this.dialogOpen=true;
  },
  ...
  async EditBatch() {
    ...
    try {
      ...
      const payload = {
        email: this.selectedBatchemail, // Assuming you have an ID field to identify the record
        batch: this.currentBatch
      };

      // Make API call to update the batch number
      const response = await this.$api.put('/api/updateBatch', payload);

      if (response.data) {
        this.showMsg('Batch number updated successfully', 'positive');
        this.getBatchData(); 
        this.closeDialog(); 
      } else {
        this.showMsg(response.data?.message || 'Failed to update batch number', 'negative');
      }
    } catch (error) {
      this.showMsg(error.response?.data.message || error.message, 'negative');
    } finally {
